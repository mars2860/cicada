...
uint8_t PMW3901::readProdId()
{
  return registerRead(0x00);
}

uint8_t PMW3901::readRevId()
{
  return registerRead(0x01);
}
...
void PMW3901::readMotionCount(int16_t *deltaX, int16_t *deltaY, uint8_t *squal)
{
  ...
  */
  ...
  registerFastRead(0x02);

  *deltaX = ((int16_t)registerFastRead(0x04) << 8) | registerFastRead(0x03);
  *deltaY = ((int16_t)registerFastRead(0x06) << 8) | registerFastRead(0x05);
  *squal = registerFastRead(0x07);
...
uint8_t PMW3901::registerFastRead(uint8_t reg) {
  reg &= ~0x80u;
