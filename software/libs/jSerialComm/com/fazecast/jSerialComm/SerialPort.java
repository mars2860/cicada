...
public class SerialPort
{
	...
	static private boolean cleanUpOnShutdown = false, allowOpenForEnumeration = false;
	...
	static
	{
		try
		{
			...
			String[] architectures = new String[0];
	...
	{
		// Usually the android device has one usb port thus for android devices we return first device connected to usb
		if (isAndroid)
		{
			libraryLock.lock();
			try
			{
				SerialPort ports[] = AndroidPort.getCommPortsNative();
